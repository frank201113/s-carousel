 ...
 */
export function setContainerStyle(wrap, opts) {
 ...
 * @param length number
 */
export function setListStyle(list, opts, isHorizontal, length) {
 ...
 * @param length number
 */
export function setItemsStyle(items, opts, isHorizontal, length) {
 ...
 * @param length 子元素数量
 */
export function cloneBoundary(items, list, length) {
    var firstItem = items[0].cloneNode(true);
    var lastItem = items[length - 1].cloneNode(true);
 ...
 * @returns number
 */
export function getVisualIndex(offset, eleSize, length) {
    var index = Math.round(Math.abs(offset) / eleSize);
    if (index === 0) {
        // 补位到最前面的那一屏
        index = length;
    }
 ...
 */
export function clearStyle(wrap, list, items, isHorizontal) {
    // 1. 父容器样式
    wrap.style.display = '';
    wrap.style.width = '';
    wrap.style.height = '';
...
export function isNonNegativeNumber(field, value) {
    var checked = typeof value === 'number' && value >= 0 && value !== Infinity;
    if (!checked) {
        throw new Error("".concat(field, " option must be a non-negative number"));
    }
}
/**
 * 判断是否是布尔值
...
export function isBoolean(field, value) {
    if (value !== true && value !== false) {
        throw new Error("".concat(field, " option must be a boolean"));
    }
}
/**
 * 为配置参数设置默认值并检测是否符合要求
