...
var SCarousel = /** @class */ (function () {
    function SCarousel(opts) {
        this.wrap = null;
        this.list = null;
        this.items = null;
        this.length = 0;
        ...
        this.observerObj = {}; // 内部索引
        checkOpts(opts);
        this.wrap = typeof opts.el === 'object' ? opts.el : document.getElementById(opts.el); // 父容器
        if (!this.wrap) {
            throw new TypeError('el is an illegal dom element');
        }
        this.list = this.wrap.children[0]; // 列表
        this.items = this.list.children; // 子元素
        this.length = this.items.length; // 子元素数量
        this.isHorizontal = opts.direction === 'left' || opts.direction === 'right';
        this.pagePos = this.isHorizontal ? 'pageX' : 'pageY';
        this.translate = this.isHorizontal ? 'translateX' : 'translateY';
        this.observerObj = { _innerActive: (opts.activeIndex || 0) + 1 };
        Object.defineProperty(this.observerObj, 'innerActive', {
            configurable: false,
            enumerable: true,
            get: function () {
                return this.observerObj._innerActive;
            },
            set: function (value) {
                this.observerObj._innerActive = value;
                value > 0 && value < length + 1 && opts.onChange && opts.onChange(value - 1);
            }
        });
    }
    SCarousel.prototype.go = function (target) {
        console.log(target);
    };
    SCarousel.prototype.start = function () { };
    SCarousel.prototype.stop = function () { };
    SCarousel.prototype.continue = function () { };
    SCarousel.prototype.resize = function (width, height) {
        console.log('resize width ', width);
        console.log('resize height ', height);
    };
    SCarousel.prototype.destroy = function () { };
    return SCarousel;
}());
// function SCarousel(this: SCarouselInstance, opts: SCarouselProps) {
//   if (!(this instanceof SCarousel)) {
//     throw new TypeError('SCarousel must be called by the \'new\' keyword as a constructor');
//   }
//   const _this = this;
//   // 参数校验
//   checkOpts(opts);
